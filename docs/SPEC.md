...
| **メディアプレビュー** | 画像サムネイル＋動画4シーンダイジェスト再生（バックグラウンドQThread、ノンブロッキング停止） |
...
| **メディアプレビュー** | ツールチップ内サムネイル | 画像ファイルはプレビュー表示、動画ファイルは4シーンダイジェスト再生（バックグラウンドスレッド処理） |
...
- 動画ファイルにホバー時にサムネイルを表示する機能を実装
  - 対応形式: MP4, AVI, MKV, MOV, WMV, FLV, WebM, M4V, MPG, MPEG, 3GP, OGV, M2TS, MTS 等（15+形式）
  - 注: `.ts` はTypeScriptファイルとの競合により除外（MPEG TSは`.m2ts`/`.mts`を使用）
